...
**v0.1.0 / 实验性首版**：用于实验室演练和分析员辅助研判。检测规则需要按业务基线调优；风险分是优先级，不是失陷概率。
...
本次交付验证：67 项 Python/接口测试、4 项 Pi 运行时测试、React 生产构建，以及桌面/移动浏览器流程检查。测试覆盖过期/撤销情报、未知认证状态、时序关联、授权篡改、重放阻断、策略变更、TTL 脚本、模拟执行/回滚、接口鉴权、Host/Origin 约束、Quake 分页/范围过滤与脱敏。
