...
**v0.4.0 / 应急手册与证据复核版**：用于实验室演练和分析员辅助研判。检测规则需要按业务基线调优；风险分是优先级，不是失陷概率。云脉接入尚待真实租户验证。安全修复与验证记录见 [安全审查报告](docs/SECURITY_REVIEW_v0.3.0.md)。

![应急手册工作台](docs/runbooks.png)

[应急手册使用指南](docs/INVESTIGATION_RUNBOOKS.md) · [基线设计与使用指南](docs/BEHAVIOR_BASELINES.md) · [v0.3.0 事件研判界面](docs/console.png)
...
| 应急手册 | 7 类流程、21 项原创检查；关联规则、人工确认系统类型、证据绑定、复核历史、报告及待审核基线复盘；参考两份应急笔记 |
...
参考 [NOP Team Linux 手册](https://github.com/Just-Hack-For-Fun/Linux-INCIDENT-RESPONSE-COOKBOOK) 与 [Bypass007 实战笔记](https://github.com/Bypass007/Emergency-Response-Notes)，把调查方向接到行为告警之后。

在“应急手册”页选择事件、确认 Linux / Windows / 未知系统类型，建立清单。每项记录核验人、依据与证据；无证据时标记缺口。“发现异常”与“已核验未见异常”必须绑定本事件证据或外部证据摘要。重复建单不覆盖历史，保存有版次冲突保护。

清单覆盖主机、持久化、Web、横向移动、权限变化和外联核验；这是人工调查指导，不等同于新增这些领域的全部自动检测器。只读命令供现场参考，无手册执行接口。导出的基线复盘是待审核任务，不会修改风险评分、允许列表或云脉权限。

详见 [使用说明、来源、API 和限制](docs/INVESTIGATION_RUNBOOKS.md)。
...
v0.4.0 验证：228 项 Python/API 测试（新增 20 项）全部通过，Pi 4 项、前端 3 项通过，React 构建通过。新增调查记录的证据约束、历史持久化、并发版次保护与导出验证；详见 docs/VALIDATION.md。
