...
## v0.4.0 investigation worksheet boundaries

Runbooks are offline, original guidance, not detection rules or an execution service. A platform is explicitly declared by the analyst. Manual observations cannot update incident risk, suppressions, baseline approvals or response authorization. Positive and negative conclusions require incident-bound events or bounded external evidence descriptors; missing data remains unresolved. External hashes are self-reported, not verified by the application.

Authenticated worksheet writes reject unknown fields, foreign evidence IDs and stale revisions within an SQLite immediate transaction. Limits: 32 worksheets and 2 MiB of worksheet JSON per saved analysis, 200 revisions per worksheet, 30 event references and 10 external descriptors per review. The UI and report escape free text. The catalog has no executor, imported shell expansion or model-generated commands. Notes and external evidence do not enter Pi automatic summaries.

The shared bearer token does not authenticate each reviewer's typed name. History is appended through the API but can be altered by a database administrator; it is not a signed forensic chain of custody. Preserve independent evidence and access records. See `docs/INVESTIGATION_RUNBOOKS.md`.
