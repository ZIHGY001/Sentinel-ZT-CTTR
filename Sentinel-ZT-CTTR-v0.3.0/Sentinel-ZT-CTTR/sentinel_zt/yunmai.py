...
from .engine import fresh_behaviors


def _text(value, field):
    if (not isinstance(value, str) or not value.strip() or value != value.strip()
            or len(value) > 160 or any(ord(c) < 32 for c in value)):
...
def handoff(analysis, config, now):
    ...
    max_age = config.get("evidence_max_age_seconds", 900)
    behavior_by_host = fresh_behaviors(analysis, now, max_age)
    review_before = now + timedelta(seconds=config.get("review_ttl_seconds", 600))
    items = []
    for incident in analysis["incidents"]:
        ...
        behavior = behavior_by_host.get(incident["host"])
        if not behavior:
        ...
        if binding and not gates:
            item["review_before"] = iso(min(review_before,
                timestamp(binding["verified_at"]) + timedelta(seconds=config.get("mapping_max_age_seconds", 3600)),
                timestamp(behavior["valid_until"])))
            item["behavior_evidence_ids"] = behavior["evidence_ids"]
            item["proposal"] = {
    ...
    document = {
        ...
        "review_before": iso(min([review_before] + [timestamp(x["review_before"]) for x in items if "review_before" in x])),
