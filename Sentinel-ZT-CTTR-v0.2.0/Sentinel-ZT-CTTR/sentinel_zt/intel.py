...
from .common import digest, integer, iso, read_bytes, timestamp
...
class Indicator:
    ...
    def parse(cls, row):
        ...
        if not isinstance(tags, (list, tuple)) or any(not isinstance(x, str) for x in tags):
    ...
    def __init__(self, indicators, now, min_confidence=60):
        ...
        for item in indicators:
            if (item.source, item.id) in revoked:
                ...
                if key not in seen:
                    self.index.setdefault((item.type, item.value), []).append(item)
                    seen.add(key)
