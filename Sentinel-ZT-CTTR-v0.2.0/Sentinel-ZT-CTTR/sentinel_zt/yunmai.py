...
def validate(config):
    ...
    for name, lo, hi, default in [("mapping_max_age_seconds", 60, 86400, 3600),
        ...
        integer(config.get(name, default), lo, hi)
...
def handoff(analysis, config, now):
    ...
    for incident in analysis["incidents"]:
        ...
        behavior_findings = [f for f in analysis["findings"]
                             if f["host"] == incident["host"] and f["rule_id"].startswith(("B", "C"))]
        behavior_ids = {eid for f in behavior_findings for eid in f["evidence_ids"]}
