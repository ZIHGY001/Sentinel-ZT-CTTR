...
<h1>从行为证据，到有边界的响应。</h1><p>关联威胁情报、端点行为与访问上下文。每个结论都能追溯证据，每次处置都受授权、时效与资产范围约束。</p></header><main>'''
    ...
    content += f'<footer>生成时间：{esc(analysis["generated_at"])}<br>策略摘要：{esc(analysis["policy_digest"])}<br>Sentinel-ZT-CTTR 0.2.1 · 本地离线报告 · 日志可能含敏感字段，请按事件材料管理。</footer></main></body></html>'
