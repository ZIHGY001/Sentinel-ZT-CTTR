...
除了新鲜的实际对端 IOC，封禁计划必须引用同资产完整的新鲜 B 类行为证据。复合链中的新 IOC 不能刷新过期行为；聚合规则的所有引用事件都需要满足时效。计划记录行为规则、证据 ID 和最早观测时间，审批、获取日志锁后及执行前都会重新检查。v0.2.1 及更早计划缺少这些字段，须重新分析、生成计划和审批；不要手工补字段延长旧计划。

执行重放检查以验证过签名的审计历史为依据，同时检查动作缓存。即使 `actions` 缓存记录缺失，只要原签名历史还在，也不会重新执行同一动作。回滚要求缓存状态与签名历史一致；不一致时停止，保全整个状态目录后人工排查。此检查不解决审计尾部截断或完整替换，需要继续外部留存 `head_mac`。

回滚在请求删除 nft 表前落盘 `rollback_intent` / `rollback_pending`；命令超时或失败落盘 `rollback_uncertain`，不会宣称恢复成功。需检查实际内核状态后决定是否重试。若表已不存在或不能读取，仍需人工确认，不自动修改为已回滚。已完成回滚可安全重复查询；预检已确认未请求变更的动作返回 `not_applied`。

## v0.3.0 无 IOC 研判与执行有效期

基线高风险行为无 IOC 也能进入人工调查和云脉交接。现有 nftables 自动适配器仍只执行有有效 IP IOC 支持、通过资产与行为检查的本机动作。新版动作携带 `intel_evidence`；过期/撤销/低置信度或与对端、观测不一致的 IOC 不能支持审批。多个匹配记录中选择情报与观测共同可用期较长的一项，绑定其实际事件，不将不同观测时间与情报寿命拼接。

审批截止时间取请求寿命、计划、行为/网络证据、绑定 IOC 和相关基线有效期的最小值。执行前仍按实时钟重检。该检查限制何时可以开始处置；执行后的内核封禁持续时间按已审批 TTL 控制。签名快照不意味着持续查询在线撤销状态，应在情报或业务上下文变化时重新研判。

升级至 v0.3.0 必须重新生成、审批旧计划。响应数据库/锁文件/密钥拒绝硬链接、非普通文件和不安全权限；不再直接 chmod 接纳不安全数据库。初始化失败释放已持有锁与连接，原审计历史不被自动删除。
